...
from src.prepare_data.output_generator import (
    Generator,
    ReferenceSummaryGenerator,
    ModelGenerator,
)
from src.prepare_data.preference_scorers import get_preference_scorer
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "config_path", type=str, help="relative path to configuration file"
    )
    parser.add_argument(
        "target_path",
        type=str,
    ...
    ref_summary_generator = get_summary_generator(
        str(ref_model_path), config.validation.generation
    )
    win_rate_calculator.set_ref_generator(ref_summary_generator)
    win_rate_calculator.ref_generate()

    # Save reference generations
    ref_output_out_filename = (
        out_path
        / "outputs"
        / f"{config.validation.ref_model_path.replace('/', '_')}.json"
    )
    ...
    if win_rate_calculator.ref_responses is not None:
        write_json_atomic(
            ref_output_out_filename, win_rate_calculator.ref_responses.to_list()
        )
        print(f"Reference outputs saved to {ref_output_out_filename}")

    for j, target_model_path in enumerate(target_paths):
        steps = (
            int(str(target_model_path).split("-")[-1])
            if "checkpoint-" in str(target_model_path)
            else -1
        )
        name = str(target_model_path.parent.name)
        if steps in [x.get("step") for x in results.get(name, [])]:
            print(
                f"Skipping target model {target_model_path}'s checkpoint {steps} as results already exist."
            )
            continue
        print(
            f"Processing target model {j + 1}/{len(target_paths)}: {target_model_path}"
        )
        target_summary_generator = get_summary_generator(
            str(MODEL_ROOT / target_model_path), config.validation.generation
        )
        ...
        target_output_out_filename = (
            out_path / "outputs" / f"{str(target_model_path).replace('/', '_')}.json"
        )
